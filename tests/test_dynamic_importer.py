...
def test_import_mod_from_lib_success_new_import(importer, mock_module):
...
def test_import_mod_from_lib_success_cached_import(importer, mock_module):
...
def test_import_mod_from_lib_failure(importer):
    with patch(
...
def test_import_mod_from_lib_returns_any(importer):
    # Simulate importing a non-module object (though importlib.import_module typically returns modules)
    mock_object = 'not a module'
    with patch.dict('sys.modules', {'yet_another_library.some_object': mock_object}):
