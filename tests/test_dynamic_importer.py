...
class TestImporter(unittest.TestCase):
    ...
    def test_import_mod_from_lib_success_new_import(self):
        with patch(
            'importlib.import_module', return_value=self.mock_module
        ) as mock_import:
            module = self.importer.import_mod_from_lib('test_library', 'test_module')
            self.assertEqual(module, self.mock_module)
            self.assertIn('test_module', self.importer.imported_modules)
            self.assertEqual(
                self.importer.imported_modules['test_module'], self.mock_module
            )
    ...
    def test_import_mod_from_lib_success_cached_import(self):
        ...
        with patch('importlib.import_module') as mock_import:
            module = self.importer.import_mod_from_lib(
                'another_library', 'cached_module'
            )
            self.assertEqual(module, self.mock_module)
    ...
    def test_import_mod_from_lib_failure(self):
        with patch(
            ...
            with self.assertRaisesRegex(
                ImportError, 'Could not import nonexistent_library.missing_module'
            ):
                self.importer.import_mod_from_lib(
    ...
    def test_import_mod_from_lib_returns_any(self):
        ...
        with patch.dict(
            'sys.modules', {'yet_another_library.some_object': mock_object}
        ):
            result = self.importer.import_mod_from_lib(
                'yet_another_library', 'some_object'
            )
